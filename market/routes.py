...
from market.static.lms import lmsFile
...
@app.route('/upload-lms')
def upload_lms():
    clear_uploads_folder()
    return render_template('upload_lms.html')
...
@app.route('/process-lms', methods=['POST'])
def process_lms():
    if 'file' not in request.files:
        return handle_file_upload_error('no_file')
    ...
    try:
        app.logger.info(f"Processing LMS file at {filepath}")
        lmsFile(filepath)  # Call the LMS processing function
        app.logger.info("LMS file processed successfully")
    except Exception as e:
        app.logger.error(f"Error processing LMS file: {e}")
        return handle_file_upload_error('processing_failed')
